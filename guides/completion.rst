...
*Tiled printing* provides the ability to print a scaled drawing that is larger than a printer's available paper format.  Tiled printing is useful where a drawing needs to be printed at a scale that would otherwise require a large format printer.  The drawing is printed across several pages that can be assembled into a large single document.

Tile printing uses "Format", "Orientation", and "Number of pages" found on the "Paper" tab of the :ref:`Drawing Preferences <draw-prefs>`.  It lays out the paper horizontally and vertically in a grid pattern to form a large format *page*.  For example, a portrait A4 page with 2 pages horizontally and 1 vertically would result in a *page* of 420 x 297 mm, or a landscape A4 in a 2 x 2 pattern would be a *page* of 594 x 420 mm.  Note that both examples use a margin of 0.  If margins are defined, the assembled document is reduced by the margin widths on the edge of the paper where they are assembled.
...
#. Click the **Calculate number of pages...** icon |icon14| from the toolbar.  The print preview will be shown the multiple pages placed side by side and the drawing in the center of it.  Note that the *Number of pages* may be on the "Paper" tab of the :ref:`Drawing Preferences <draw-prefs>`
...
#. Switch to the print preview window and check the completed drawing.  If necessary, click "Zoom All" and then "Fit to Page" to align the drawing to the page.
#. Click the **Print** icon |icon01| or select **File -> Print**.
#. Select the printer on the *Print* dialogue and confirm the properties by clicking the **Properties** icon.  Adjust the properties if necessary and then click the **Print** button.
...
.. admonition:: Tip - Setting the Margins
    ...
    Set the margins to the printer's minimum margins when using tiled printing to maximize the assemble print size.  The drawing margins can be defined by using a border if required.
