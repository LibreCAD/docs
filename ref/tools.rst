...
The drawing tools are used to create and modify entities such as lines, circles, etc. in a drawing.  Commands can be selected from the menu, toolbars or entered via the :ref:`command line <cmdline>`. Right-click anywhere in the drawing space pops a small window showing the history of commands launched with the menu or the toolbars, last command being on top of the list. Note that command line input are not shown.
